      ...
      <div className='relative z-10 pt-[60px] lg:pt-[100px]'>
        <div ref={sectionRef} className='flex flex-col items-start gap-5 px-6 lg:px-20'>
          <Badge
            ...
            style={{
              color: (FEATURE_TABS[activeTab] as typeof FEATURE_TABS[0] & { badgeColor?: string }).badgeColor ?? FEATURE_TABS[activeTab].color,
              backgroundColor: hexToRgba(
                (FEATURE_TABS[activeTab] as typeof FEATURE_TABS[0] & { badgeColor?: string }).badgeColor ?? FEATURE_TABS[activeTab].color,
                0.1
              ),
            }}
          >
            ...
            <div role='tablist' aria-label='Feature categories' className='flex flex-1'>
              {FEATURE_TABS.map((tab, index) => (
                <button
                  ...
                  className={`relative h-full flex-1 items-center justify-center whitespace-nowrap px-3 font-medium font-season text-[var(--landing-text-dark)] text-[12px] uppercase lg:px-0 lg:text-sm${(tab as typeof tab & { hideOnMobile?: boolean }).hideOnMobile ? ' hidden lg:flex' : ' flex'}${index > 0 ? ' border-[var(--divider)] border-l' : ''}`}
                  style={{ backgroundColor: index === activeTab ? '#FDFDFD' : '#F6F6F6' }}
                >
                  {(tab as typeof tab & { mobileLabel?: string }).mobileLabel ? (
                    <>
                      <span className='lg:hidden'>{(tab as typeof tab & { mobileLabel?: string }).mobileLabel}</span>
                    ...
                    <div className='absolute right-0 bottom-0 left-0 flex h-[6px]'>
                      {tab.segments.map(([opacity, width], i) => (
                        <div
                          ...
                          style={{ width: `${width}%`, backgroundColor: tab.color, opacity }}
                        />
                      ))}
                    </div>
                  )}
          ...
          <div className='mt-8 flex flex-col gap-6 px-6 lg:mt-[60px] lg:grid lg:grid-cols-[1fr_2.8fr] lg:gap-[60px] lg:px-[120px]'>
            <div className='flex flex-col items-start justify-between gap-6 pt-5 lg:h-[560px] lg:gap-0'>
              <div className='flex flex-col items-start gap-4'>
                ...
                <p className='font-[430] font-season text-[color-mix(in_srgb,var(--landing-text-dark)_50%,transparent)] text-base leading-[150%] tracking-[0.02em] lg:text-lg'>
