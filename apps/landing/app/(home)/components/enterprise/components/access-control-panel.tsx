...
function FeatureToggleItem({ feature, enabled, color, isInView, delay, textClassName, onToggle }: FeatureToggleItemProps) {
  return (
    <motion.div
      role='button'
      tabIndex={0}
  ...
  return (
    <div ref={ref}>
      {PERMISSION_CATEGORIES.map((category, catIdx) => {
        const offsetBefore = PERMISSION_CATEGORIES.slice(0, catIdx).reduce((sum, c) => sum + c.features.length, 0)
        return (
            ...
            <div className='mt-2 grid grid-cols-2 gap-x-4 gap-y-2'>
              {category.features.map((feature, featIdx) => (
                <FeatureToggleItem
                  ...
                  onToggle={() => setAccessState((prev) => ({ ...prev, [feature.key]: !prev[feature.key] }))}
                />
              ))}
            </div>
          </div>
