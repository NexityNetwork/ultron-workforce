...
const ENTERPRISE_FEATURE_MARQUEE_STYLES = `
  ...
  @media (prefers-reduced-motion: reduce) {
    .enterprise-feature-marquee-track { animation: none; }
  }
`
        ...
        <div className='mt-8 overflow-hidden rounded-[12px] bg-[var(--landing-bg)] sm:mt-10 md:mt-12'>
          ...
          <div className='enterprise-feature-marquee relative mt-6 overflow-hidden border-[var(--landing-bg-elevated)] border-t'>
            ...
            <div className='enterprise-feature-marquee-track flex w-max'>
              {[...FEATURE_TAGS, ...FEATURE_TAGS, ...FEATURE_TAGS, ...FEATURE_TAGS].map((tag, i) => (
                <span
                  key={i}
                  className='whitespace-nowrap border-[var(--landing-bg-elevated)] border-r px-5 py-4 font-[430] font-season text-[color-mix(in_srgb,var(--landing-text-subtle)_60%,transparent)] text-[13px] leading-none tracking-[0.02em] hover:bg-white/[0.04] hover:text-[color-mix(in_srgb,var(--landing-text-subtle)_80%,transparent)]'
                >
                  {tag}
                </span>
              ))}
