...
export default function Navbar({ logoOnly = false, blogPosts = [] }: NavbarProps) {
  ...
  return (
    ...
    >
      <Link href={logoHref} className={LOGO_CELL} aria-label='Sim home' itemProp='url'>
        <span itemProp='name' className='sr-only'>Sim</span>
        <Image
        ...
        <>
          <ul className='mt-[0.75px] hidden lg:flex'>
            {NAV_LINKS.map(({ label, href: rawHref, external, icon, dropdown }) => {
              const href =
                isBrowsingHome && rawHref.startsWith('/#') ? `/?home${rawHref.slice(1)}` : rawHref
...
function NavChevron({ open }: { open: boolean }) {
  return (
    <svg width='9' height='6' viewBox='0 0 10 6' fill='none' className='mt-[1.5px] flex-shrink-0'>
      <line
        x1='1' y1='1' x2='5' y2='5'
        stroke='currentColor' strokeWidth='1.33' strokeLinecap='square'
        style={{
